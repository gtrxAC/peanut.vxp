...
typedef VMINT (*vm_sm_get_sym_entry_t)(char* symbol);

/************************************************************************/

extern VMUINT _get_mre_sym_entry(void);
#define MRE_ENTRY ((vm_sm_get_sym_entry_t)_get_mre_sym_entry())
...
#define SM_PROCESS_BEGIN() {\
	vm_sm_get_sym_entry_t mre_entry = MRE_ENTRY;	\
	if (((int_int_t)mre_entry("vm_sm_set_sb_by_ptr"))((VMINT)handle_smevt) == 0) {

#define SM_PROCESS_RETURN() ((void_void_t)mre_entry("vm_sm_reset_sb"))();

#define SM_PROCESS_END() SM_PROCESS_RETURN() } }

#define SM_CALLBACK_BEGIN() {\
	vm_sm_get_sym_entry_t mre_entry = MRE_ENTRY;	\
	if (((int_void_t)mre_entry("vm_sm_set_sb_for_invoker"))() == 0) { 
...
#define SM_INVOKE_BEGIN(SM)  {\
	vm_sm_get_sym_entry_t mre_entry = MRE_ENTRY;	\
	if (((int_int_t)MRE_ENTRY("vm_sm_set_sb_by_handle"))(SM) == 0) {
